...
```{r include=FALSE}
knitr::opts_chunk$set(warning=FALSE, echo=FALSE, message=FALSE, out.width="9 0%")
```
...
- Acessar um conjunto de páginas específicas
    - Pode ser **complicado**: conhecimento de requisições web
- A partir de uma página HTML, extrair os dados de interesse
...
- Tidyverse (`dplyr`, `tidyr`, `stringr`, `purrr`, `lubridate`)
- Expressões regulares (`regex`)
- `XPath` e `CSSPath`

## Conceitos básicos

- Requisição GET: `httr::GET()`
- Requisição POST: `httr::POST()`
- Inspect element
